// Service worker for MRT Station Guesser.
//
...
async function fillShell() {
  ...
  if (failed.length) {
    console.warn(
      `[mrtguessr] ${failed.length} of ${PRECACHE.length} precache entries failed. ` +
...
self.addEventListener("fetch", (event) => {
  ...
  // Rounds and the leaderboard: network only, and the page says so offline.
