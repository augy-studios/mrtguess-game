...
--sql writes the same rows as a new numbered file in migrations/, for pasting
into the SQL editor instead of running this against the API. It never
overwrites an existing migration: 002 was the first, and a refresh after that
becomes the next number.
...
def next_seed_path() -> Path:
    ...
    return MIGRATIONS / f"{max(numbers, default=0) + 1:03d}_mrtguessr_seed_stations.sql"
...
def write_sql(rows: list[dict], path: Path) -> None:
    ...
    path.write_text(
        ...
        "-- Do not edit; a later refresh is a new file. Updates stations by English\n"
        "-- name and never deletes one.\n\n"
