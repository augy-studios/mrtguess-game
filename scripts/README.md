...
| Script | What it does |
|---|---|
| `seed_supabase.py` | Loads `main-site/data/stations.geojson` into `mrtguessr_stations`. Upserts on the English name, so it is safe to run again. `--dry-run` prints what it would send; `--sql` writes the same rows as the next numbered file in `migrations/` instead, never overwriting one. |
